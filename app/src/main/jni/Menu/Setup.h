...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Subway Surf Mod Menu</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">Modded by bakkaq</p> | "
                                     "Mod menu template by LGL | thanks to PlatinMods</p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("Modded by bakkaq :P"),ToastLength::LENGTH_LONG);
